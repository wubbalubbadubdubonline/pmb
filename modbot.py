...
async def on_ready():
	...
	try:
		...
		pass
	norole = [x for x in client.get_server("181866934353133570").members if len(x.roles) < 2]
	if norole:
		for x in norole:
			role = [x for x in x.server.roles if x.name.lower() == "guest"]
			if role:
				await client.add_roles(x, role[0])
